...
local CacheManagement = {}
CacheManagement.__index = CacheManagement
...
local RequireFromURL = function(url)
...
function CacheManagement:DownloadWithIntegrityCheck(url)
    ...
    for _, fileData in pairs(manifest) do
        ...
        writefile("MercenaryX/Modules/" .. computedHash, fileResponse)
    end

    return true
end

function CacheManagement:Load(filePath)
    local chunk, err = loadfile(filePath)
    if not chunk then
        return nil, stringformat("(Mercenary X) [Load] Failed to load file \"%s\": %s", filePath, err)
    end

    local success, result = pcall(chunk)
    if not success then
        return nil, stringformat("(Mercenary X) [Load] Error running file \"%s\": %s", filePath, result)
    end
